...
`qr-only:verify` confere hash, tamanho bruto, tamanho gzip e redução em relação ao baseline. `qr-only:test` chama o leitor compilado de verdade para validar o contrato mínimo. `qr-only:parity` fornece onze QRs públicos e sintéticos como os mesmos pixels RGBA para o baseline e para o artefato QR-only: versões 1/L, 5/H, 10/Q e 20/M, ASCII, UTF-8 rotacionado, bytes binários invertidos, todos os 256 valores de byte e capturas degradadas por contraste/iluminação, blur, quantização lossy pós-blur e perspectiva. A quantização modela pixels depois de uma compressão de imagem sem armazenar JPEGs ou documentos no Git. O gate exige igualdade exata dos bytes retornados.
